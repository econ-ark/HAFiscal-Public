...
2. **Portable Path System**: `\latexroot` macro system enables context-aware path resolution
...
├── _path-to-parent.ltx.tex                       # Path resolution configuration
...
The system uses `\latexroot` to provide relative path resolution from any directory:

```latex
% In Figures/_path-to-parent.ltx.tex
\providecommand{\latexroot}{}\renewcommand{\latexroot}{..}
\providecommand{\PathsToInputs}{}\renewcommand{\PathsToInputs}{_paths-to-inputs.ltx}
\input{\latexroot/\PathsToInputs}
```
...
```latex
\providecommand{\CodeDir}{\latexroot/Code}
\providecommand{\DataDir}{\latexroot/Data}  
\providecommand{\FigsDir}{\latexroot/Figures}
\providecommand{\TableDir}{\latexroot/Tables}
\providecommand{\ApndxDir}{\latexroot/Appendices}
```
...
### Subfiles Directory
- Each subfile loads `\latexroot` and path definitions
...
- **Update `_path-to-parent.ltx.tex`** if directory relationships change
...
**Solutions**:
1. Check `_path-to-parent.ltx.tex` relative path from current directory
...
grep -r "latexroot" @local/                # Check path definitions
```
