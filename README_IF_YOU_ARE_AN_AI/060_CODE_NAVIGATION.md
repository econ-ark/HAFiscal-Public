...
│
└── Empirical/                      # Stata empirical analysis
    ├── make_liquid_wealth.do       # SCF data processing
...
|------|---------|---------|
| `Code/HA-Models/reproduce_min.py` | Minimal validation run | ~1 hour |
...
```stata
# Run in Stata:
do Code/Empirical/make_liquid_wealth.do
```
...
| File | Purpose |
|------|---------|
| `make_liquid_wealth.py` | Python version of Stata processing |
...
| Process SCF data | Run `Code/Empirical/make_liquid_wealth.do` in Stata |
