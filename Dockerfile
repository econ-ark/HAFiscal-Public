...
# This ensures venv is built for the correct architecture (x86_64 or arm64)
RUN echo '#!/bin/bash' > /home/vscode/entrypoint.sh && \
    echo 'set -e' >> /home/vscode/entrypoint.sh && \
    echo '' >> /home/vscode/entrypoint.sh && \
    echo '# Determine platform-specific venv path' >> /home/vscode/entrypoint.sh && \
    echo 'PLATFORM=$(uname -s | tr "[:upper:]" "[:lower:]")' >> /home/vscode/entrypoint.sh && \
    echo 'if [ "$PLATFORM" = "darwin" ]; then' >> /home/vscode/entrypoint.sh && \
    echo '    VENV_PATH="/workspace/.venv-darwin"' >> /home/vscode/entrypoint.sh && \
    echo 'else' >> /home/vscode/entrypoint.sh && \
    echo '    VENV_PATH="/workspace/.venv-linux"' >> /home/vscode/entrypoint.sh && \
