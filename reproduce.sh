...
if [[ "${TERM:-}" == "dumb" ]]; then
    ...
    if is_capable_terminal; then
        ...
        echo ""
    # Third check: Is this an automation environment?
...
benchmark_end() {
    ...
    # Ensure benchmarks directory exists (write to auto/ subdirectory)
    local benchmark_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/reproduce/benchmarks/results/auto"
...
EOF
        ...
        # Create/update latest symlink in auto/ directory
        ...
        ln -sf "auto/$filename" "$parent_dir/latest.json"
        
        echo ""
        log INFO "Benchmark saved: reproduce/benchmarks/results/auto/$filename"
...
ensure_uv_environment() {
    ...
    if [[ -n "${VIRTUAL_ENV:-}" ]]; then
        ...
        echo "========================================"
        echo "⚠️  Virtual Environment Path Mismatch"
        echo "========================================"
        echo ""
        echo "A virtual environment is active, but it appears to be from a different"
        echo "location (possibly created on a different machine or container)."
        echo ""
    ...
    if [[ -f "$expected_venv/bin/python" ]]; then
        ...
        local actual_python
        actual_python=$(which python3 2>/dev/null || which python 2>/dev/null || echo "")
