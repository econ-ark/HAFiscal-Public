...
# Detect platform-specific venv path (reproduce_environment_comp_uv.sh creates platform-specific venvs)
# On Linux: .venv-linux, on macOS: .venv-darwin, fallback: .venv
PLATFORM_VENV=""
case "$(uname -s)" in
    Darwin)
        PLATFORM_VENV=".venv-darwin"
        ;;
    Linux)
        PLATFORM_VENV=".venv-linux"
        ;;
    *)
        PLATFORM_VENV=".venv"
        ;;
...
if [ -d "$PLATFORM_VENV" ] && [ -f "$PLATFORM_VENV/bin/python" ]; then
    ...
    echo "✅ Found platform-specific virtual environment: $PLATFORM_VENV"
